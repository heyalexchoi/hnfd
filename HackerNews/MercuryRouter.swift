...
enum MercuryRouter: URLRequestConvertible {
    ...
    func asURLRequest() throws -> URLRequest {
        let url = URL(string: Private.Constants.parserAPIURLString)!.appendingPathComponent(path)
        var request = try URLEncoding.default.encode(URLRequest(url: url), with: parameters)
        request.httpMethod = method.rawValue
        request.addValue(Private.Keys.parserAPIKey, forHTTPHeaderField: "x-api-key")
        return request
    }
}
